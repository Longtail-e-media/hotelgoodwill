...
if ($menuRec):
    ...
    foreach ($menuRec as $menuRow):
        ...
        if (!empty($data)):
            $linkActive = ($menuRow->linksrc == $data) ? " status-active" : "";
            $parentInfo = Menu::find_by_linksrc($data);
            if ($parentInfo):
                $PlinkActive = ($menuRow->id == $parentInfo->parentOf) ? " status-active" : "";
            endif;
        endif;
        ...
        $result .=  '<li class="'.$linkActive . $PlinkActive.'">';
...
if ($menuRec):
    ...
    foreach ($menuRec as $menuRow):
        ...
        $drop1 = !empty($menusubRec) ? ' <i class=icon-down-open-mini></i>' : '';
        $result3 .= '<li>';
        $result3 .= getMenuFootList($menuRow->name, $menuRow->linksrc, $menuRow->linktype, $linkActive . $PlinkActive . $classLink, $chkchild);
...
$jVars['module:footer-experience'] = $result3;
// experience end
// booking
$result4 = '';
...
if ($menuRec):
    ...
    foreach ($menuRec as $menuRow):
        ...
        $drop1 = !empty($menusubRec) ? ' <i class=icon-down-open-mini></i>' : '';
        $result4 .= '<li>';
        $result4 .= getMenuFootList($menuRow->name, $menuRow->linksrc, $menuRow->linktype, $linkActive . $PlinkActive . $classLink, $chkchild);
...
$jVars['module:footer-otalinks'] = $result4;
// booking end
// bottom menu
$result5 = '';
$menuRec = Menu::getMenuByParent(0, 5);
$current_url = $_SERVER["REQUEST_URI"];
$data = explode('/', $current_url);
...
if ($menuRec):
    ...
    foreach ($menuRec as $menuRow):
        ...
        $drop1 = !empty($menusubRec) ? ' <i class=icon-down-open-mini></i>' : '';
        $result5 .= '<li>';
        $result5 .= getMenuFootList($menuRow->name, $menuRow->linksrc, $menuRow->linktype, $linkActive . $PlinkActive . $classLink, $chkchild);
...
$jVars['module:bottom-menu'] = $result5;
// bottom menu end
