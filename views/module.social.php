...
* Top Social Links
*/
$socialRec = SocialNetworking::getSocials();
// pr($socialRec );
...
if(!empty($icon)){
    $disicon .='<li><a href="' . $socialRow->linksrc . '" target="_blank"><img src="'.IMAGE_PATH.'social/' . $socialRow->image . '"/></a></li>';
}
else{
    $disicon .='<li><a href="' . $socialRow->linksrc . '" target="_blank"><i class="' . $socialRow->icon . '"></i></a></li>';
}
...
// pr($resocl);
$jVars['module:socilaLinkbtm'] = $disicon;
...
if (!empty($socialRec)) {
    foreach ($socialRec as $socialRow) {
        $resslss = ' <a href="' . $socialRow->linksrc . '" target="_blank"><i class="fa ' . $socialRow->image . '"></i></a>';
    }
}
$jVars['module:socilaLinktop'] = $resslss;
