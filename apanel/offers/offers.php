    ...
    <div class="example-box">
      ...
      <div class="form-row add-image">
                    ...
                    <div class="form-input col-md-10 uploader">
                        <input type="file" name="offer_upload_multi" id="offer_upload_multi" class="transparent no-shadow">
                        <label>
                    ...
                    <!-- Upload user image preview -->
                    <div id="preview_Image_slider"><input type="hidden" name="image_offers[]"/></div>
                    <?php
                    if (!empty($advInfo->multiimage)) {
                        $imgRec = unserialize($advInfo->multiimage);
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                $deleteid = rand(0, 99999);
                                $imagePath = SITE_ROOT . 'images/offers/offerimg/' . $recimg;
                            ...
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/offers/offerimg/" . $recimg)):
                                    $filesize = filesize(SITE_ROOT . "images/offers/offerimg/" . $recimg);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                            ...
                                            <img src="<?php echo IMAGE_PATH . 'offers/offerimg/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
                                            <input type="hidden" name="image_offer[]" value="<?php echo $recimg; ?>"
            ...
            <!-- multiple images  end -->
                ...
                <!--<div class="form-row">
                    ...
                    <div class="form-input col-md-20">
                        <textarea placeholder="Brief" class="col-md-6" type="text"
                                  name="brief"
                                  id="brief"><?php echo !empty($advInfo->brief) ? $advInfo->brief : ""; ?></textarea>
                    </div>
                </div>
<!-- content brief end -->
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            // new one  multiple image
            $('#offer_upload_multi').uploadify({
                'swf': '<?php echo ASSETS_PATH;?>uploadify/uploadify.swf',
                'uploader': '<?php echo ASSETS_PATH;?>uploadify/uploadify.php',
                'formData': {
                    PROJECT: '<?php echo SITE_FOLDER;?>',
                    targetFolder: 'images/offers/offerimg/',
                    thumb_width: 200,
                    thumb_height: 200
                },
                'method': 'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageName').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/offers/uploaded_image_2.php', {imagefile: filename}, function (msg) {
