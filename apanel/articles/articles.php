...
if (isset($_GET['page']) && $_GET['page'] == "articles" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    ...
    SerclearImages($moduleTablename, $moduleFoldername.'/slider','multiimage');
    SerclearImages($moduleTablename, $moduleFoldername . "/slider/thumbnails",'multiimage');
    ?>
    <h3>
        List Articles
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
        ...
        <div class="example-code">
            ...
            <div class="form-row add-image">
                    ...
                    <div class="form-input col-md-10 uploader">
                        <input type="file" name="gallery_upload_multi" id="gallery_upload_multi" class="transparent no-shadow">
                        <label>
                        <small>Image Dimensions (1180px X 900px)
                        </small>
                    </label>
                    </div>
                    <!-- Upload user image preview -->
                    <div id="preview_Image_slider"><input type="hidden" name="image_Slider[]"/></div>
                    <?php
                    if (!empty($articlesInfo->multiimage)) {
                        $imgRec = unserialize($articlesInfo->multiimage);
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                $deleteid = rand(0, 99999);
                                $imagePath = SITE_ROOT . 'images/articles/slider/' . $recimg;
                            ...
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/articles/slider/" . $recimg)):
                                    $filesize = filesize(SITE_ROOT . "images/articles/slider/" . $recimg);
                                ...
                                ?>
                                            ...
                                            <img src="<?php echo IMAGE_PATH . 'articles/slider/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
                                            <input type="hidden" name="image_Slider[]" value="<?php echo $recimg; ?>"
                ...
                <div class="form-row add-image">
                    ...
                    <div class="form-input col-md-10 uploader">
                        <input type="file" name="gallery_upload" id="gallery_upload" class="transparent no-shadow">
                            <label>
                            <small>Image Dimensions (1920px X 1000px)
                            </small>
                        </label>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#gallery_upload_multi').uploadify({
                'swf': '<?php echo ASSETS_PATH;?>uploadify/uploadify.swf',
                'uploader': '<?php echo ASSETS_PATH;?>uploadify/uploadify.php',
                'formData': {
                    PROJECT: '<?php echo SITE_FOLDER;?>',
                    targetFolder: 'images/articles/slider/',
                    thumb_width: 200,
                    thumb_height: 200
                },
                'method': 'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageName').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/articles/uploaded_image_multi.php', {imagefile: filename}, function (msg) {
                        $('#preview_Image_slider').append(msg).show();
                    });
