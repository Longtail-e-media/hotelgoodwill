...
class Offers extends DatabaseObject
{
    ...
    protected static $db_fields = array(
        ...
        'status', 'added_date', 'sortorder', 'type', 'list_image', 'multiimage');
    ...
    public static function get_internal_link($Lsel = '', $LType = 0)
    {
        ...
        if ($pages):
            ...
            foreach ($pages as $pageRow):
                $sel = ($Lsel == ("offer-details/" . $pageRow->slug)) ? 'selected' : '';
                $result .= '<option value="offer-details/' . $pageRow->slug . '" ' . $sel . '>&nbsp;&nbsp;' . $pageRow->title . '</option>';
            endforeach;
            $result .= '</optgroup>';
        endif;
        return $result;
